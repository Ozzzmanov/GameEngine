...
/**
 * Клас `NodeTreePanel` реалізує відображення та управління ієрархією вузлів сцени.
 * Панель дозволяє:
 * - Відображати дерево вузлів з їх дочірніми елементами та 3D-моделями;
 * - Додавати нові вузли та видаляти існуючі;
 * - Перейменовувати вузли через контекстне меню;
 * - Обробляти вибір вузлів та моделей, синхронізуючи дані з редактором.
 *
 * Основні функції:
 * - Рендеринг структури сцени у вигляді дерева з можливістю розгортання/згортання вузлів.
 * - Забезпечення інтерактивного інтерфейсу для роботи з вузлами (додавання, видалення, перейменування).
 * - Інтеграція з редактором через реалізацію інтерфейсу `EditorListener` для отримання подій.
 *
 * Основні методи:
 * - `renderContent()` – відображає основний вміст панелі, включаючи кнопки для додавання та видалення вузлів,
 *   а також викликає рекурсивний метод `renderNodeTree(Node node)` для рендерингу дерева.
 * - `renderNodeTree(Node node)` – рекурсивно відображає вузли, моделі та їх контекстні меню.
 * - Методи з інтерфейсу `EditorListener` (`onNodeSelected`, `onSelectionCleared`, `onSceneChanged`) для
 *   синхронізації стану панелі з редактором.
 *
 * @author Вадим Овсюк
 * @version 0.8
 * @since 2025-03-23
